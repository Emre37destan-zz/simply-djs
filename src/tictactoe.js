...
async function tictactoe(message, options = []) {
    try {
        ...
        if(options.slash === true){
            ...
            if (options.credit === false) {
                foot = options.embedFoot || 'Mutlaka kazanın ;)'
            } else {
            ...
            message.followUp({
              content: 'Merhaba <@' + opponent.id + '>. tic tac toe isteğin var',
                embeds: [acceptEmbed],
                components: [accep]
            })
                ...
                collector.on('collect', async (button) => {
                    if (button.customId == 'declinettt') {
                        ...
                        const xoemb = new Discord.MessageEmbed()
                            .setTitle('TicTacToe')
                            .setDescription(`**Nasıl oynanır ?**\n*Sıranızı bekleyin.. Sıra sizdeyse, emojinizi oraya çizmek için tablodaki düğmelerden birine tıklayın.*`)
                            .setColor(options.embedColor || 0x075FFF)
                            .setFooter(foot)
                            .setTimestamp()
                        let infomsg = message.editReply({ embeds: [xoemb] })
        
                        let msg = await message.channel.send({ content: `Giriş Bekleniyor | <@!${Args.userid}>, Emojiniz: ${o_emoji}` })
                        tictactoe(msg)
                        ...
                        async function tictactoe(m) {
                            ...
                            if (won["<:O_:863314110560993340>"] != false) {
                                if (Args.user == 0) return m.edit({ content: `<@!${fighters[1]}> (${o_emoji}) kazandı.. Güzel bir oyundu.`, components: [] }); else if (Args.user == 1) return m.edit({ content: `<@!${fighters[0]}> (${o_emoji}) kazandı.. Güzel bir oyundu.`, components: [] });
                            }
                            ...
                            if (won["<:X_:863314044781723668>"] != false) {
                                if (Args.user == 0) return m.edit({ content: `<@!${fighters[1]}> (${x_emoji}) kazandı.. Güzel bir oyundu.`, components: [] }); else if (Args.user == 1) return m.edit({ content: `<@!${fighters[0]}> (${x_emoji}) kazandı.. Güzel bir oyundu.`, components: [] });
                            }
                            ...
                            m.edit({ content: `Giriş Bekleniyor | <@!${Args.userid}> | Emojiniz: ${Args.user == 0 ? `${o_emoji}` : `${x_emoji}`}`, components: [a, b, c] })
                            ...
                            collector.on('collect', b => {
                                ...
                                if (b.user.id !== Args.userid) return b.reply({ content: 'Şansını bekle.', ephemeral: true })
                                ...
                                if (Object.keys(Brgs).length == 0) return m.edit({ content: 'Bir kravat!', components: [] })
                                tictactoe(m)
                            });
                            collector.on('end', collected => {
                                if (collected.size == 0) m.edit({ content: `<@!${Args.userid}> zamanında tepki vermedi! (30s)`, components: [] })
                            });
                        }
    ...
    if (!opponent) return message.channel.send({ content: "Lütfen kullanıcıya meydan okumasını sağlayın!" })

    if (opponent.id === message.member.id) return message.channel.send({ content: "2 Oyuncu için oynayamazsınız. Lütfen kullanıcıya meydan okumasını sağlayın!" });
    ...
    console.log(`Hata oluştu. | tiktaktoe | Hata: ${err}`)
}
