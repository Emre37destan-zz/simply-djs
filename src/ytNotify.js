...
async function ytNotify(client, db, options = []) {
    ...
    if (!chid) throw new Error('EMPTY_CHANNEL_ID. Kanal kimliği belirtmediniz. Destek almak için https://discord.com/invite/AnUXS6z5tY adresine gidin');
    if (!options.ytID && !options.ytURL) throw new Error('EMPTY_YT_CHANNEL_ID & EMPTY_YT_CHANNEL_URL. Kanal kimliği belirtmediniz. Destek almak için https://discord.com/invite/AnUXS6z5tY adresine gidin');
 try {
    ...
    setInterval(async () => {
        ...
        let msg = options.msg || 'Merhaba ! **{author}** az önce yeni bir video yükledi **{title}**\n\n*{url}*'
    ...
    console.log(`Hata oluştu. | ytBildir | Hata: ${err}`)
}
