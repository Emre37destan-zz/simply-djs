...
async function btnrole(client, message, options = []) {
    try {
            if (!options.data) throw new Error('NO_DATA_PROVIDED. Düğme yapmak için herhangi bir veri belirtmemişsiniz..')

            let { MessageButton, MessageActionRow } = require('discord.js')

            if (!message.author.permissions.has('ADMINISTRATOR')) return message.reply('Bu eğik çizgi komutunu kullanmak için `YÖNETİCİ` iznine ihtiyacınız var')
            ...
            if (data.length <= 5) {
                ...
                throw new Error('Maksimum 25 rol kabul edildi.. Bunun aşılması hatalara neden olur.')
            }
            async function btnroleengin(data, button, row) {
                ...
                if (!options.embed) throw new Error('NO_EMBED_SPECIFIED. Bana göndermek için herhangi bir embed belirtmediniz ..')
        ...
        console.error(`Hata Oluştu | btnrol | . Hata: ${err}`)
    }
